...
function VehicleVINNumber() {
    const [inputValue, setInputValue] = useState();

    console.log(inputValue);

    const vehicles = VehicleJson();
    const vinNum = vehicles.map(vehicle => vehicle.vinNumber);
    const vinNumberList = [...new Set(vinNum)];
    ...
    return (
        <div className="radio margin_top">
            <Autocomplete
                ...
                color="secondary"
                renderInput={(params) => <TextField {...params} label="VIN номер" />}
            />
                ...
                <div>
                    <ul>{uniqueVehicles.map((elem) => elem.vinNumber === inputValue ? <div className="cardVehicle" key={elem.id}>
...
export default VehicleVINNumber;